...
public class VTEmulator extends AEmulator
{
   ...
   /* ------------------------------------------------------------
    ...
    VTEmulator(VTMapper map, ATranslator tran)
    {  super(map,tran);
    ...
    public boolean doChar(char c)
    {
        ...
        switch (state)
        {
            ...
            case STATE_DCA2:
                if (term != null)
                {  term.setCursorPos(dca_line,(c - ' ')); }
                reset_state();
                return  false;
            default:
                state = STATE_ASCII;
                System.out.println("jTerminal error: *** Unknown VT100 emulator state!");
                return  false;
        }
    }
   ...
   /* ------------------------------------------------------------
    ...
    void doESC(VTCommand mycmd)
    {
                    ...
                    if (state == STATE_DATA)
                    {
                        switch (cmd.type)
                        {
                            ...
                            case STATE_APC:
                                ...
                                return;
                            default:
                                System.out.println("jTerminal error: Unknown state at string terminator.");
                                reset_state();
                                return;
                        }
                    }
                    System.out.println("jTerminal error: String terminator outside of string.");
                    return;
        ...
        System.out.println("jTerminal error: Unknown C0.ESC: " + mycmd.toString());
        System.out.println("jTerminal error: term: " + mycmd.term);
        System.out.println("jTerminal error: inters: " + new String(mycmd.inters,0,mycmd.cur_inter));
    }
    
    /** Parse C1.CSI. */
    void doCSI(VTCommand mycmd)
    {
        int mop = -1;
        switch (mycmd.term)
        {
        ...
        if (mop >= 0)
        {
            ...
            for (int i = 0; i < mycmd.cur_param; i++)
            {
                ...
                if (j >= VTMode.N)
                    System.out.println("jTerminal error: Unknown Mode: " +
        ...
        if (mycmd.flags == 0 && mycmd.cur_inter <= 0)
        {
            ...
            switch (mycmd.term)
            {
                ...
                case 'c':           // DA: Device attr report
                    ...
                    {
                        System.out.println("jTerminal error: Unknown DA: "+ mycmd.toString());
                    }
                    break;
                case 'g':
                    if (term != null)
                ...
                case 'y':           // DECTST: Invoke confidence test
                    ...
                    break;
                default:
                    System.out.println("jTerminal error: Unknown C1.CSI: "+mycmd.toString());
                    break;
            }
            return;
        }
        ...
        if (mycmd.cur_param >= 1 && mycmd.cur_inter <= 0)
        {
            ...
            {  scmd.setType(C1.CSI);
               ...
               scmd.setTerminator('n'); }
                ...
                System.out.println("jTerminal error: Unknown DSR: " + mycmd.toString());
        }
        else
            System.out.println("jTerminal error: Unknown DSR: " + mycmd.toString());
    ...
    void doDCS(VTCommand mycmd)
    {
        ...
        if (mycmd.flags == 0 && mycmd.cur_inter <= 0)
        {
            ...
            {
                default:
                    System.out.println("jTerminal error: Unknown C1.DCS: "+ mycmd.toString());
                    break;
            }
            return;
        }
    ...
    void doOSC(VTCommand mycmd)
    {
        ...
        if (mycmd.flags == 0 && mycmd.cur_inter <= 0)
        {
            ...
            {
                default:
                    System.out.println("jTerminal error: Unknown C1.OSC: "+ mycmd.toString());
                    break;
            }
            return;
        }
    ...
    void doPM(VTCommand mycmd)
    {
        ...
        if (mycmd.flags == 0 && mycmd.cur_inter <= 0)
        {
            ...
            {
                default:
                    System.out.println("jTerminal error: Unknown C1.PM: "+ mycmd.toString());
                    break;
            }
            return;
        }
    ...
    void doAPC(VTCommand mycmd)
    {
        ...
        if (mycmd.flags == 0 && mycmd.cur_inter <= 0)
        {
            ...
            {
                default:
                    System.out.println("jTerminal error: Unknown C1.APC: "+ mycmd.toString());
                    break;
            }
            return;
        }
...
//       System.out.println(""+VTMode.abbrvs[md]+"="+VTMode.names[md]+":"+op);
       switch (op)
       {
           ...
           case VTMode.RESTORE:
               ...
               break;
           default:
               System.out.println("jTerminal error: Unknown mode operation: " + op);
       }
       if (md == VTMode.DECOM)
       {
           if (term != null)
