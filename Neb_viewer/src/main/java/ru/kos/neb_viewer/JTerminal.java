...
public class JTerminal extends JPanel implements ATerminal
{
    ...
    final static class COLOR
    {
        final static Color
                ...
                DARK_BLUE = new Color((0x00 << 16) | (0x00 << 8) | 0x80),
...
//        int size = 18, width, height, ascent, descent;
...
//            }
    ...
    final public JView view = new JView();
    final class JView extends JPanel
    {
        ...
        {
            int i = rely + offset;
            Row row = buffer.rows[rely + offset];
            return row;
        }
...
//            for (int y = clip.fm.y; y <= clip.to.y; y++)
    ...
    final public JScroll scroll = new JScroll();
...
//                    {
//                        scroll.setValue(max_scroll_position);
//                        System.out.println("max_scroll_position keyPressed="+max_scroll_position+" scroll="+scroll.getValue());
//                    }
//                });
                view.addMouseWheelListener(new MouseWheelListener()
                {
                    public void mouseWheelMoved(MouseWheelEvent e)
                    {
                        ...
                        if(n < 0)
                        {
                            if(val > 0) scroll.setValue(val-3);
//                            System.out.println("n="+n+" val="+(val-1));
                        }
                        else
                        {
                            if(val < 3000) scroll.setValue(val+3);
//                             System.out.println("n="+n+" val="+(val+1));
                        }
                    }
                });
        ...
        void pressed(int relx, int rely)
        { /* Paint Clip */
            if (fill.fm.x >= 0)
                clip.set(fill);
            else
    ...
    { /* buffer.format = (short)((buffer.format& 0xF00F) | ((color& 0xF) <<8)); */
    }
    public void setBackColor(int color)
    { /* buffer.format = (short)((buffer.format& 0x0F0F) | ((color& 0xF) <<12)); */
    }
